...
struct AudioPacket {
    ...
    long long absOffset = 0;  // Absolute byte position in the stream
};
...
public:
    ...
    // Bytes are copied into the internal StreamReader window, which is trimmed
    // by deferred compaction on the following call.
