// IMkvReader implementation used by WebmDemuxer.
//
//   StreamReader — append-only window with explicit compaction. mkvparser needs
//                  random access by absolute position, which a consuming SPSC
//                  stream cannot provide, so bytes are pumped in from
//                  WebMStreamBuffer and trimmed via compact().
//
